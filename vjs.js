...
let data, senteces = [], decryptSrc = null, lines = {}, track;
...
function startPlayer() {
    ...
    if (iOS && track) {
        ...
        fetch(track.file).then(e => e.text()).then(text => {
            let arr = text.split("\n");
            arr.push("");
...
function initPlayer() {
    ...
    if (!iOS && track) {
        data.tracks.forEach(e => player.addRemoteTextTrack({ kind: "captions", label: e.label, src: e.file, language: e.language, default: e == track, mode: e == track ? "showing" : "hidden" }, false));
...
<div>
${arr.map(e => `
<div style="a" class="vjs-text-track-cue">
    <div style="color: rgb(255, 255, 255); background-color: rgb(0, 0, 0); position: relative; inset: 0px; display: inline; writing-mode: horizontal-tb; unicode-bidi: plaintext; font-family: sans-serif;">${e}</div>
</div>`).join("")}
</div>`;
